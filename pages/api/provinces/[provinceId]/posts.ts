...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!provinceId || Array.isArray(provinceId)) return res.status(400).json({ error: 'Invalid provinceId' });
  const pid = parseInt(provinceId as string, 10);
  if (isNaN(pid)) return res.status(400).json({ error: 'provinceId is not a number' });
